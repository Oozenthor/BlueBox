...
SerialDialog::SerialProfile SerialDialog::profile() const // Return a valid communication port set of values
{
  return currentProfile;
}

void SerialDialog::fillPortsInfo() // Collect the manufacturer information for each connected serial device.
{
...
void SerialDialog::showPortInfo(int idx) // Show the manufacturer information for each connected serial device.
{
  if (idx != -1) {
...
void SerialDialog::fillPortsParameters()  // Fill the list boxes with all possible serial port settings
{
...
void SerialDialog::updateProfile()  // Read all list boxes and set the communication profile based on the contents.
...
void SerialDialog::saveProfileToDisk()  // Read current list box values and save them to disk
{
...
void SerialDialog::loadProfileFromDisk()  // Read saved profile and restore all list box selections
{
  QSettings setting("SerialProfile.ini", QSettings::IniFormat);
  setting.beginGroup("SerialProfile");
...
void SerialDialog::on_applyButton_clicked()  // Accept current selection set
{
  updateProfile();
  saveProfileToDisk();
  hide();
...
void SerialDialog::on_refreshButton_clicked() // Refresh port list (new ports might be added i.e. USB to serial)
{
  fillPortsInfo();
}
